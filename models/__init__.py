...
def cbam_resnet50(in_channels, num_classes, pretrained=False):
    model = ptcv_get_model('cbam_resnet50', pretrained=False)
    # model.output = nn.Linear(2048, 7)
    return model


def cbam_resnet34(in_channels, num_classes, pretrained=False):
    model = ptcv_get_model('cbam_resnet34', pretrained=False)
    return model


def bam_resnet50(in_channels, num_classes, pretrained=False):
    model = ptcv_get_model('bam_resnet50', pretrained=False)
    model.output = nn.Linear(2048, 7)
    return model
